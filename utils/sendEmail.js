...
exports.errorFromWeb = (printerId, shortcode, error, browser, country) => {
    /**
    ...
    const data = {
      ...
      message: `Error desde la página web, alguien ha intentado acceder a la información de la impresora con id <b>${printerId}</b> y ha habido un error al traer la información del shortcode ${shortcode} y está dando el siguiente error:<br/><div style="padding: 10px; background-color: #cecece;">${error}</div><br/> country: ${country ||'No hay información del país'} <br/> browser: ${browser}`,
      type: 'error'
    }
    email.sendEmail(data);
}
